    ...
    $( window ).scroll(function() {
      ...
      $(".timeline-wrapper").each(function() {
        ...
        if (isElementInView($('#' + current.find('.timeline-event')[0].id), true) ||
            isElementInView($('#' + current.find('.timeline-event')[1].id), true) ) {
          openEvent($('#' + current.find('.timeline-event')[0].id), $('#' + current.find('.timeline-event-content')[0].id));
          openEvent($('#' + current.find('.timeline-event')[1].id), $('#' + current.find('.timeline-event-content')[1].id));
        } else {
          if (current.find('a').is('.open')) {
            closeEvent($('#' + current.find('.timeline-event')[0].id), $('#' + current.find('.timeline-event-content')[0].id));
            closeEvent($('#' + current.find('.timeline-event')[1].id), $('#' + current.find('.timeline-event-content')[1].id));
          }
        }
      });
    });
