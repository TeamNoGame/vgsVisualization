...
(function ($) {
  $.timeliner = function (options) {
    ...
    $(document).ready(function () {
      for (let i = 0; i < $.timeliners.options.length; i++) {
  ...
  function startTimeliner(options) {
    const settings = {
      ...
      // note: changing this selector from the default will require modifications to the CSS file in order to retain
      // default styling
      ...
      // note: changing this tag from the default will require modifications to the CSS file in order to retain default
      // styling
      ...
      // default: dt
      // note: When leaving this value at its default, you do not need to apply a class to the dt element if you use the
      // plugins recommended tag structure and markup
      // note: Change this from the default, perhaps to a class like ".timeline-event", in the case that you do not want
      // to use the plugins recommened markup structure and prefer to use anothe element (e.g, div) instead of a dt tag
      // to mark each event within a series.
      // note: Changing this value from the default will require modifications to the CSS file in order to retain
      // default styling

      timelineEXContent: options.timelineEXContent || '.timeline-event-content',
      // Element that contains the event's full content to be displayed when event is expanded, an event's expanded ID
      // should alway be on this item
      // value: class selector
      // default: .timeline-event
      // note: changing this selector from the default will require modifications to the CSS file in order to retain
      // default styling
      ...
      // sets whether only one item on the timeline can be open at a time. If true, other items will close when one is
      // opened.
      // value: true | false
      // default: false
      ...
      // note: the flat state is an initial display option only -- the timeline major markers return to normal once
      // they've been opened/displayed

      startOpen: options.startOpen || ['.start-open'],
      // As of version 2.3, you can simply add the "start-open" class to each timeline-event you want to have open by
      // default; see the demo source code or code sample below
    ...
    $(window).scroll(function () {
      ...
      $('.timeline-wrapper').each(function () {
        const current = $(this);
        for (let i = 0; i < current.find('.timeline-event').length; i++) {
          if (
            isElementInView($(`#${current.find('.timeline-event')[i].id}`), true) ||
            isElementInView($(`#${current.find('.timeline-event')[i].id}`), true)) {
            openEvent(
    ...
    // Minor Event Click
    $(settings.timelineContainer).on('click', `${settings.timelineTriggerContainer} ${settings.timelineEventContainer}`,
        function () {
      ...
      if ($(this).find('a').is('.open')) {
        ...
        if (settings.oneOpen == true) {
          closeEvent(
            $(this).parents(settings.timelineContainer).find(settings.timelineTriggerAnchor,
              settings.timelineTriggerContainer),
            $(this).parents(settings.timelineContainer).find(settings.timelineEXContent)
          );
        }
    ...
    $(settings.timelineContainer).on('click', settings.timelineSectionMarker, function () {
      ...
      const numEvents = $(this).parents(settings.timelineSection).find(settings.timelineTriggerContainer).length;

      // number of minor events already open
      const numOpen = $(this).parents(settings.timelineSection).find('.open').length;

      // This closes other items if oneOpen is true. It looks odd if an item in the section was open. Need to improve
      // this.
      if (settings.oneOpen === true) {
        closeEvent( $(this).parents(settings.timelineContainer).find(settings.timelineTriggerAnchor,
          settings.timelineTriggerContainer),
        $(this).parents(settings.timelineContainer).find(settings.timelineEXContent)
        );
      }
      ...
      if (numEvents > numOpen) {
        ...
        openEvent(
          $(this).parents(settings.timelineSection).find(settings.timelineTriggerAnchor,
            settings.timelineTriggerContainer),
          $(this).parents(settings.timelineSection).find(settings.timelineEXContent)
        );
      } else {
        closeEvent(
          $(this).parents(settings.timelineSection).find(settings.timelineTriggerContainer),
          $(this).parents(settings.timelineSection).find(settings.timelineEXContent)
        );
      }
    });
    ...
    // All Markers/Events
    const el = `${settings.timelineContainer} ` + '.timeline-toggle';
    $(el).click(function () {
      if ($(el).hasClass('expanded')) {
        closeEvent(
          $(el).parents(settings.timelineContainer).find(settings.timelineTriggerAnchor,
            settings.timelineTriggerContainer),
          $(el).parents(settings.timelineContainer).find(settings.timelineEXContent)
        );
...
/*
      ...
      // var windowScrollMod = windowScrollTop % windowHeight;
      //
      // console.log('Window Height:', windowHeight);
      // console.log('Window Scroll:', windowScrollTop);
      // console.log('Document Height:', documentHeight);
      //
      // if ( windowScrollMod === 0 || (windowScrollTop + windowHeight) === documentHeight) {
      //
      // }
 */
